...
public class TestsEx3 {
	...
	private static void graphConditions() throws IOException {
		...
		prettyPrintTEcore(d_gc, "GraphCondition", diagrams);
		...
		for (TGraphMorphism m : pm.getMatches()) {
			...
			prettyPrintTEcore(match, label, diagrams);
	...
	public static TGraph loadBoardAsTGraph(ResourceSet rs, String fileName, String label) throws IOException {
		return loadBoardAsTGraphs(rs, fileName, label)[0];
	}
	...
	private static void ecoreFormalisationAsTypedGraphs() throws IOException {
		...
		prettyPrintEcore(d.getGraphDiagram(), "TrelloInstanceTyped", diagrams);
	...
	private static void ecoreFormalisationAsGraphs() throws IOException {
...

		{
			...
			d.objects(importer.getResult());
			prettyPrintEcore(d, "Ecore", diagrams);
		}

		{
			...
			GraphDiagram d = new GraphDiagram();
			d.objects(importer.getResult());
			prettyPrintEcore(d, "SimpleTrello", diagrams);
		}

		{
			...
			GraphDiagram d = new GraphDiagram();
			d.objects(importer.getResult());
			prettyPrintEcore(d, "TrelloInstance", diagrams);
		}
	}
